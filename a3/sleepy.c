	    ...
	    loff_t *f_pos)
{
  ...
  if (minor < 0 || minor >= sleepy_ndevices)
    return -EINTR;
  printk("SLEEPY_READ DEVICE (%d): Process is waking everyone up. \n", minor);
  sleep_number = 1;

  wake_up_interruptible(&dev->sleepers);
	     ...
	     loff_t *f_pos)
{
  ...
  if (count == 4)
  {
    ...
    input = kmalloc(sizeof(input),GFP_KERNEL);
    err = copy_from_user(input, buf, count);
    //printk("Input is: %d\n", *input);

    //unsigned long j, alarm;
    //j = jiffies;
    //alarm = HZ * (*input);
    //if (!time_before(j, alarm))
    
    if (*input > 0)
    {
      sleep_number = 0;
      mutex_unlock(&dev->sleepy_mutex);
      retval = wait_event_interruptible_timeout(dev->sleepers, sleep_number != 0, HZ * (*input));
      retval /= HZ;
    }
    else
    {
			...
			struct class *class)
{
  ...
  init_waitqueue_head(&dev->sleepers);
